...
public class OrderClient extends RestClient {
    ...
    public ValidatableResponse getIngredients() {
        return given().spec(spec).when().get(INGREDIENTS_PATH).then();
    }

    @Step("Create Order, burger ingredients {ingredients} ")
    public ValidatableResponse createOrder(Ingredients ingredients) {
        return given().spec(spec).body(ingredients).when().post(ORDERS_PATH).then();
    }
    ...
    public ValidatableResponse createOrderAuthorized(Ingredients ingredients, Authorized auth) {
        return given().spec(spec).header("Authorization", auth.getAccessToken()).body(ingredients).when().post(ORDERS_PATH).then();
    }

    @Step("get orders of user {auth} ")
    public ValidatableResponse getAuthorizedUserOrders(Authorized auth) {
        return given().spec(spec).header("Authorization", auth.getAccessToken()).when().get(ORDERS_PATH).then();
    }

    @Step("get orders, unauthorized  ")
    public ValidatableResponse getUnauthorizedUserOrders() {
        return given().spec(spec).header("Authorization", "").when().get(ORDERS_PATH).then();
    }
}
